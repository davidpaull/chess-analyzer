...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--centipawn', action='store_true', help='Show Centipawn graphs...')
    parser.add_argument('-e', '--evalgraph', action='store_true', help='Show Eval graph after calculations...')
    ...
    if args.evalgraph:
        ...
        plt.plot(eval_graph)
        ##
        eval_min = abs(min(eval_graph))
        eval_max = abs(max(eval_graph))
        if eval_min > eval_max:
            eval_max = eval_min
        eval_min = eval_max * -1
        ##
        plt.ylim(ymin=eval_min - 100, ymax=eval_max + 100)
        plt.title('Game Evaluation')
        plt.show()

    if args.centipawn:

        # centipawn loss by move
        from matplotlib import pyplot as plt
        w_plt = plt.plot(cp_res['white'], color='orange', label='White')
        b_plt = plt.plot(cp_res['black'], color='black', label='Black')
        plt.title('Centipawn Loss by move')
        ...
        # average centipawn loss for move to end (for all moves)
        w, b = proc_cp(cp_res)

        # Now let's make some charts :D
        from matplotlib import pyplot as plt
        w_plt = plt.plot(b, color='black', label='black')
        b_plt = plt.plot(w, color='orange', label='white')
        plt.title('Centipawn Loss by move number to end')
        plt.ylabel('Centipawn Loss')
        plt.xlabel('Move Number')
        plt.xlim(xmin=0.0)
        plt.ylim(ymin=0.0)
